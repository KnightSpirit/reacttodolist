...
class TodoTitle extends React.Component {
	...
	render() {
		return (
			<div>
			<span className="todolistheader" style={{display:this.state.titleDisplay}}>{this.state.headtext} </span>
			<input type="text" style={{display:this.state.inputdisplay}} onChange={this.handleChange.bind(this)} />
			<span onClick={this.ModifyTitle.bind(this)}>{this.state.buttonText}</span>
			</div>);
	}
}
